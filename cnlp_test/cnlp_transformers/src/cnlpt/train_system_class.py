...
def main(
    ...
    def build_compute_metrics_fn(
        ...
        def compute_metrics_fn(p: EvalPrediction):
            logits = p.predictions[0]           # shape: (N, C)
            probs = softmax(logits, axis=1)     # shape: (N, C)
            labels = p.label_ids                # shape: (N,) or (N, 1)
            ...
            # 为单任务情况将task metrics提升到顶层，以便Trainer的metric_for_best_model能识别
            if len(task_names) == 1:
                task_name = task_names[0]
                task_metrics = metrics[task_name]
                for key, value in task_metrics.items():
                    if key not in metrics:  # 避免覆盖已存在的顶层metrics
                        metrics[key] = value
    ...
    trainer = Trainer(
        ...
        callbacks=[EvalCallback(),
                   EarlyStoppingCallback(early_stopping_patience=training_args.early_stopping_patience, early_stopping_threshold=training_args.early_stopping_threshold),]
    )
    ...
    if training_args.do_eval:
            ...
            if training_args.load_best_model_at_end:
                ...
                if model_name == "cnn":
                    ...
                    model = model_type.from_pretrained(
                        ...
                        task_names=task_names,
                        num_labels_dict=num_labels,
                        embed_dims=model_args.cnn_embed_dim,      # 传递实际参数
                        num_filters=model_args.cnn_num_filters,   # 传递实际参数
                        filters=model_args.cnn_filter_sizes,      # 传递实际参数
                    )
                elif model_name == 'lstm':
                    model = model_type.from_pretrained(
                        model_path,
                    ...
                    logger.info("Using current hierarchical model for prediction")
        ...
        # 保存best_eval结果到单独文件，方便超参数筛选
        if trainer.is_world_process_zero():
            best_eval_file = os.path.join(training_args.output_dir, "best_eval_results.json")
            if hasattr(model, "best_eval_results"):
                # 使用训练过程中保存的最佳结果
                best_results = model.best_eval_results
                logger.info("Using best_eval_results from training")
            else:
                # 如果没有训练过程中的最佳结果，使用当前的eval_result
                best_results = eval_result
                logger.info("Using current eval_result as best_eval_results (no training best found)")
            
            with open(best_eval_file, "w") as f:
                json.dump(best_results, f, indent=2)
            logger.info(f"Best eval results saved to {best_eval_file}")

    if training_args.do_predict:
        logging.info("*** Test ***")
        trainer.compute_metrics = None

        test_dataset = dataset.processed_dataset["test"]
        
        # 直接进行预测而不使用trainer.evaluate()来避免覆盖wandb summary
        test_predictions = trainer.predict(test_dataset=test_dataset)
        
        # 手动计算test metrics而不通过trainer.evaluate()
        compute_metrics_fn = build_compute_metrics_fn(task_names, None, dataset)  # 传入None避免wandb记录
        test_result = compute_metrics_fn(test_predictions)

        '''test_dataset = dataset.processed_dataset["test"]
    
        # 使用相同的evaluate逻辑
        if not hasattr(model, "best_test_results"):
            trainer.compute_metrics = build_compute_metrics_fn(
                        task_names, model, dataset
                    )
            test_result = trainer.evaluate(eval_dataset=test_dataset)
        else:
            test_result = model.best_test_results'''
        ...
        if trainer.is_world_process_zero():
            for dataset_ind, dataset_path in enumerate(data_args.data_dir):
                ...
                ) # returned the evalpredictions class from Huggingface {predictions: list([class logits]), nparray(label_id)}
                  # class logits example: predictions=[array([[ 0.30922544,  0.07401957, -0.09435967],.....]
                  # label_id shape: (num_data, 1): label_ids=array([[0.],
                ...
                # for classification task, task_to_label_packet return the  (preds, labels, prob_values)
                # preds shape: {number of data}: (1036,)
                # labels shape: {number of data}
                # if output_prob == False, prob_values shape: (), else (1036,)
                out_table = process_prediction(
                    task_names=dataset.tasks,
