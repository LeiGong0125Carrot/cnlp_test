...
class CnlpTrainingArguments(TrainingArguments):
    ...
    early_stopping_patience: int = field(
        default=5,
        metadata={
            "help": "Number of evaluation calls to wait for improvement before early stopping."
        }
    )
    early_stopping_threshold: float = field(
        default=0.0,
        metadata={
            "help": "Minimum improvement to count as an actual improvement for early stopping."
        }
    )
    ...
    lr_scheduler_type: str = field(
        default="linear",
        metadata={
            "help": "The scheduler type to use. Choose from ['linear', 'cosine']."
        }
    )
