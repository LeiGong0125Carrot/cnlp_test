...
def acc_and_f1(preds: np.ndarray, labels: np.ndarray) -> dict[str, Any]:
    ...
    # 为每个类别添加单独的 f1、recall、precision 和 acc_and_f1 指标
    unique_labels = np.unique(labels)
    for i, label_idx in enumerate(unique_labels):
        label_idx = int(label_idx)  # 转换为整数
        if i < len(f1):
            metrics[f"f1_class_{label_idx}"] = f1[i]
            # 为每个类别计算 acc_and_f1（准确率与该类别F1的平均值）
            metrics[f"acc_and_f1_class_{label_idx}"] = (acc + f1[i]) / 2
        if i < len(recall):
            metrics[f"recall_class_{label_idx}"] = recall[i]
        if i < len(precision):
            metrics[f"precision_class_{label_idx}"] = precision[i]
